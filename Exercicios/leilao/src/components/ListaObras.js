...
const ListaObras = () => {
  ...
  const getObras = async () => {
    ...
    db.collection("obras").orderBy("titulo").onSnapshot((dados) => {
      const docs = [];
      dados.forEach((obra) => {
  ...
  <div className="container mt-2">
    <div className="card-columns">
      {obras.map((obra) => (<ItemLista id={obra.id} titulo={obra.titulo} foto={obra.foto} autor={obra.autor} minimo={obra.minimo}/>))}
    </div>
  </div>;
